...
- Hardened nested validation so missing or malformed `inputs`/`outputs` fields consistently raise `RunManifestError`.
- Stage records now require every serialized field (`name`, status/timestamps, message, commands, inputs, and outputs).

## Tests

- `python -m unittest tests.test_run_manifest -v`: 9 tests passed (including missing nested fields).
